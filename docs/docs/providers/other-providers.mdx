...
- Codex OAuth 默认使用 OpenClaw 的参数：`https://auth.openai.com/oauth/authorize`、`https://auth.openai.com/oauth/token`，以及回调 `http://127.0.0.1:1455/auth/callback`（PKCE）。
- 若提供商参数不同，可在设置中编辑 `client_id`、`scope`、回调端口与路径。
...
- 请求需携带 `Authorization: Bearer <appToken>`，Body 包含 `appid`、`channel_type` 与手机号/邮箱。
