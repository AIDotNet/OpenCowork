...
- **Generic AnimatedIcon wrapper:** `AnimatedIcon` component in `icons/AnimatedIcon.tsx` wraps any lucide icon with `whileHover`/`whileTap` motion animations. Animation types: scale, rotate, spin, bounce, shake, pulse, wiggle, swing, flip, pop, jello, rubber. Icon-to-animation mapping in `icons/icon-animation-map.ts`. Used in `CommandPalette.tsx` via a local `anim()` helper that auto-resolves animation by icon display name. Also used in `WorkspaceSidebar.tsx` for Extensions section icons: header FolderOpen (bounce), Resources FolderOpen (bounce), Draw Image (scale), Skills Wand2 (rotate), SSH Monitor (pulse).
