...
export function AppPluginPanel(): React.JSX.Element {
  ...
  const activeState = (() => {
    if (!selectedPlugin) return 'missing'
    if (!selectedPlugin.enabled) return 'disabled'
    if (!isResolvedImageModelReady) return 'not_ready'
    return 'ready'
  })()
          ...
          <div className="space-y-6">
            ...
            <section className="space-y-3 rounded-xl border p-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t('plugin.toolStatusDesc')}</p>
              </div>
              <div className="rounded-lg border bg-muted/10 p-3">
                <p className="text-sm font-medium">{IMAGE_GENERATE_TOOL_NAME}</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t(`plugin.status.${activeState}`)}
                </p>
                <p className="mt-2 text-xs text-muted-foreground">{t('plugin.toolArgs')}</p>
            ...
            <section className="space-y-2">
              <p className="text-sm font-medium">{t('plugin.items.image.promptTitle')}</p>
              <p className="text-xs text-muted-foreground">{t('plugin.items.image.promptDesc')}</p>
            </section>
          </div>
        ) : (
