...
export function MorphPopoverContent({
  ...
  const viewportPad = 8
  const unclampedLeft = layout
    ? align === 'end'
      ? layout.trigger.left + layout.trigger.width - layout.content.width
      : layout.trigger.left
    : 0
  const belowTop = layout ? layout.trigger.top + layout.trigger.height + sideOffset : 0
  const aboveTop = layout ? layout.trigger.top - layout.content.height - sideOffset : 0
  const fitsBelow =
    !layout || belowTop + layout.content.height <= window.innerHeight - viewportPad
  const unclampedTop = layout
    ? side === 'bottom'
      ? fitsBelow
        ? belowTop
  ...
  const left = layout
    ? Math.min(
        Math.max(viewportPad, unclampedLeft),
        Math.max(viewportPad, window.innerWidth - layout.content.width - viewportPad)
      )
    : 0
  const top = layout
    ? Math.min(
        Math.max(viewportPad, unclampedTop),
        Math.max(viewportPad, window.innerHeight - layout.content.height - viewportPad)
      )
    : 0
