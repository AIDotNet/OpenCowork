...
export interface AISidebarProps {
  ...
  renderTrailing?: (item: SidebarResource) => ReactNode
...
const ResourceRow = forwardRef<HTMLDivElement, ResourceRowProps>(function ResourceRow(
  {
    row,
    active,
    expanded,
  ...
  return (
    <motion.div
      ...
      transition={
        reduce
          ? { duration: 0 }
          : { height: ROW_REVEAL, opacity: ROW_REVEAL, layout: SPRING_LAYOUT }
      }
    ...
    >
      <div
        ...
        draggable={enableMove && !row.item.disabled && !renaming}
        ...
        onContextMenu={(event) => {
          if (event.defaultPrevented || renaming || row.item.disabled || !menu) return
          event.preventDefault()
      ...
      >
        <span aria-hidden="true" className="relative grid size-5 shrink-0 place-items-center">
          <span
            className={cn(
              ...
              acceptsChildren &&
                'group-hover/resource:opacity-0 group-data-[menu-open=true]/resource:opacity-0'
            )}
          >
          ...
          {acceptsChildren ? (
            <span
              className={cn(
                'pointer-events-none col-start-1 row-start-1 grid origin-center place-items-center opacity-0 transition-[opacity,rotate] duration-200 ease-out motion-reduce:transition-none',
                'group-hover/resource:opacity-100 group-data-[menu-open=true]/resource:opacity-100',
                expanded && 'rotate-90'
              )}
            >
        ...
        {!renaming ? renderTrailing?.(row.item) : null}
        ...
        {!renaming && !row.item.disabled && menu ? (
          <MorphPopover open={menuOpen} onOpenChange={onMenuOpenChange}>
            ...
            <MorphPopoverContent
              ...
              sideOffset={8}
              radius={12}
              className={menuContentClassName ?? 'w-40 p-1.5'}
            >
...
export function AISidebar({
  ...
  const expandedIds = useMemo(
    () => (expandedControlled ? new Set(expandedIdsProp) : uncontrolledExpandedIds),
    [expandedControlled, expandedIdsProp, uncontrolledExpandedIds]
  )
  ...
  const performMove = useCallback(
    ...
    [enableMove, onMove, onMoveError, renderedItems, updateItems]
  )
  ...
  const toggle = useCallback(
    ...
    [expandedControlled, expandedIds, onExpandedChange]
  )
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLDivElement>, row: FlatResource) => {
      ...
      if (enableMove && moveModifier && event.key === 'ArrowUp' && previous) {
        event.preventDefault()
        void performMove({ itemId: row.item.id, targetId: previous.item.id, position: 'before' })
        return
      }
      if (enableMove && moveModifier && event.key === 'ArrowDown' && next) {
        event.preventDefault()
      ...
      if (
        ...
        event.preventDefault()
        if (!expandedIds.has(previous.item.id)) toggle(previous.item.id)
        void performMove({ itemId: row.item.id, targetId: previous.item.id, position: 'inside' })
        return
      }
      if (enableMove && moveModifier && event.key === 'ArrowLeft' && row.parentId) {
        event.preventDefault()
    ...
    [enableMove, expandedIds, flat, focusRow, performMove, select, toggle]
  )
